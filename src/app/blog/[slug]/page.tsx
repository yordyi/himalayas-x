...
interface PortableTextValue extends SanityImage {
  style?: string
  href?: string
  alt?: string
}
...
export async function generateMetadata({
  ...
  try {
    const post = await getPost(params.slug) as BlogPost | null

    if (!post) {
      return {
        title: 'Post Not Found - Himalayas X',
    ...
    return {
      ...
      openGraph: {
        title: post.title,
        description: post.excerpt,
        images: post.mainImage ? [image(post.mainImage).size(1200, 630).url()] : [],
      },
    }
  } catch (error) {
...
export default async function BlogPost({
  ...
  const post = (await getPost(params.slug) as BlogPost) || notFound()
